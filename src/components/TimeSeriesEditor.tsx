...
function usePinotTables(props: TimeSeriesProps): string[] {
  ...
  useEffect(() => {
    props.datasource.getResource<GetTablesResponse>('tables')
    .then(resp => setTables(resp.tables))
  }, [0])
  return tables
}
...
export function TimeSeriesQueryEditor(props: TimeSeriesProps) {
  ...
  const timeColumns = schema?.dateTimeFieldSpecs
  .map(spec => spec.name)

  const metricColumns = schema?.metricFieldSpecs
  .map(spec => spec.name)
